...
<div class="wrapper">
    ...
    <section class="content">
      ...
      <div class="box box-primary">
            ...
            <div class="row">
              <div class="col-sm-6">
                  <div class="form-group"><label>Nomor Pesanan (*)</label>
                    <?php echo form_input($nomor_pesanan) ?>
                  </div>

                  <div class="form-group">
                    <label>Generate Nomor Pesanan</label>
                    <br>
                      <?php echo form_checkbox($generate_no) ?>
                      Aktif
                  </div>
                  ...
                  <div class="form-group"><label>Nomor Resi</label>
                    <?php echo form_input($nomor_resi) ?>
                  </div>
                  ...
                  <div class="form-group"><label>Biaya Admin</label>
                    <?php echo form_input($biaya_admin) ?>
                  </div>
                  ...
                  <div class="form-group"><label>Status Transaksi (*)</label>
                    <?php echo form_dropdown('status', $get_all_status, '', $status) ?>
                  </div>
              </div>

              <div class="col-sm-6">
                  <div class="form-group"><label>Nama Penerima (*)</label>
                    <?php echo form_input($nama_penerima) ?>
                  </div>

                  <div class="form-group"><label>No. HP Penerima</label>
                    <?php echo form_input($hp_penerima) ?>
                  </div>
            ...
            <div class="row">
              <div class="col-sm-12">
                 <div class="form-group"><label>Nama Produk (*)</label>
                    <?php echo form_dropdown('produk', '', '', $produk) ?>
                  </div>
              </div>
            </div>
          </div>
          ...
          <div class="box-footer">
            <button type="submit" id="penjualan_tambah" name="button" class="btn btn-success"><i class="fa fa-save"></i> <?php echo $btn_submit ?></button>
  ...
  <script type="text/javascript">
    ...
    // submit form masuk
    $('#penjualan_tambah').click(function(e){
      ...
      if (status == '' && toko == '' && kurir == '' && ongkir == '' && no_pesanan == '' && nama_penerima == '' && provinsi == '' && alamat == '' && dt_id == '' && dt_qty == '' && dt_harga == '' && dt_jumlah == '' && dt_hpp == '' && dt_jumlah_hpp == '') {
        ...
        Toast.fire({
          icon: 'error',
          title: 'Terjadi Kesalahan!. Nama Kurir harap dipilih!'
        });
      }else if(status == ''){
        Toast.fire({
          icon: 'error',
        ...
        if (resi == '') {
          $.ajax({ 
            url:"<?php echo base_url()?>admin/keluar/penjualan_produk_proses",
            method:"post",
            dataType: 'JSON', 
          ...
          if (produk != '') {
            $.ajax({
              ...
              success: function(data){
                ...
                document.getElementById("in_jumlah_hpp").value = 0;
    ...
    $('input[type="checkbox"].minimal, input[type="radio"].minimal').iCheck({
     ...
     $('input[type="checkbox"].minimal').on('ifClicked', function() {
          ...
          if(checked){
              ...
              $.ajax({
              url: "<?php echo base_url()?>admin/keluar/generate_nomor",
              type: "post",
              data: {[csrfName]: csrfHash},
              dataType: 'JSON',
              success: function(data){
                $('#nomor-pesanan').val(data);
              },
              error: function(){
                alert('Error ....');
              }
    ...
    $('#import-penjualan').click(function(e){
      if ($.fn.DataTable.isDataTable("#table-validasi-import")) {
        $('#table-validasi-import').DataTable().clear().destroy();
        $('#table-validasi-import').dataTable().fnDestroy();
      }     
      ...
      var imports = document.getElementById('impor_penjualan');
      var csrfName = '<?php echo $this->security->get_csrf_token_name(); ?>',
      csrfHash = '<?php echo $this->security->get_csrf_hash(); ?>';
      // alert(panjangArray);
      ...
      if(imports.files['length'] == 0){
        Toast.fire({
          icon: 'error',
          title: 'Terjadi Kesalahan!. File Import Penjualan harus diisi!'
        });
      }else{
        var formData = new FormData();
        formData.append('impor_penjualan', $('#impor_penjualan')[0].files[0]);  
        ...
        $("#modal-proses").modal('show');
        $.ajax({ 
          url:"<?php echo base_url()?>admin/keluar/proses_impor_new",
          method:"post",
          dataType: 'JSON', 
          // data:{img: JS_image,vendor:vendor, penerima:penerima, sku: sku, kategori: kategori, ongkir: ongkir, remarks: remarks, nomor_request: nomor_request, dt_id: JS_id, dt_qty: JS_qty, dt_harga: JS_harga, dt_jml: JS_jumlah, dt_diskon: JS_diskon, dt_pajak: JS_pajak, length: panjangArray, [csrfName]: csrfHash},
          data: formData,
          ...
          success:function(data)  {  
            // alert(data);
            $("#modal-proses").modal('hide');
            document.getElementById("impor_penjualan").value = null;
