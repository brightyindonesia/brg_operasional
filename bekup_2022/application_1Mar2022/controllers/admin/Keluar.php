...
class Keluar extends CI_Controller {
	...
	public function __construct()
	{
	    ...
	    $this->load->model(array('Bahan_kemas_model', 'Vendor_model', 'Venmasaccess_model', 'Produk_model', 'Toko_model', 'Tokproaccess_model', 'Kurir_model', 'Keluar_model', 'Keluar_sementara_model', 'Paket_model', 'Resi_model', 'Dashboard_model', 'Status_transaksi_model'));
	...
	public function dasbor_penjualan()
	{
		...
		$this->data['get_all_toko_impor'] = $this->Keluar_model->get_all_toko_list();
		$this->data['get_all_toko_penjualan'] = $this->Keluar_model->get_all_toko_list();
		...
		if ($keyword == "no") {
			...
			if ($this->upload->do_upload('impor_penjualan')) {
					...
					if (count($cek_pesanan) >= $count_nomor_pesanan[$val_store['nomor_pesanan']]) {
						...
						if ($count_nomor_pesanan[$val_store['nomor_pesanan']] > count($cek_pesanan_sementara)) {
							...
							$storing_fix[] = array(	'nomor_pesanan'			=> $val_store['nomor_pesanan'],
													...
													'id_status_transaksi'	=> $val_store['id_status_transaksi'],
							);

							$row_produk = $this->Produk_model->get_by_id($val_store['id_produk']);
							$msg_err .= 'Nomor Pesanan: <b>'.$val_store['nomor_pesanan'].' (Sementara)</b> diganti dengan <b>'.$count_nomor_pesanan[$val_store['nomor_pesanan']].' Detail Pesanan</b> di <b>Data Sementara</b>. </br>';
							$validasi++;	
						}
				...
				if (count($storing_fix) > 0) {
						...
						if (isset($cek_nomor)) {
							...
							if (isset($cek_pesanan_produk)) {
								...
								if ($cek_pesanan_produk->ongkir == 0 AND $ongkir == 0 ){
									...
									$total_harga 		= ((int)$ongkir_fix + $harga_jual);
									$selisih_margin 	= $total_jual - $total_hpp;
									$margin 			= $total_jual - $total_harga;
					...
					if ($validasi > 0) {
						$msg = array(	'sukses'	=> $baris.' Data Berhasil Import Penjualan!',
										'pesan_error'	=> $msg_err
			    			);
				    	echo json_encode($msg);	
					}else{
	...
	public function sinkron_total_harga()
	{
		...
		$start 		 = substr($this->input->post('periodik'), 0, 10);
		$end 		 = substr($this->input->post('periodik'), 13, 24);
		$cek_pesanan = $this->Keluar_model->get_all_by_periodik_impor($start, $end);

		foreach ($cek_pesanan as $val_pesanan) {
			$cek_detail = $this->Keluar_model->get_detail_by_id($val_pesanan->nomor_pesanan);

			if (count($cek_detail) > 0) {
				$total_harga = $val_pesanan->ongkir;
				$harga_jual  = 0;
				$hpp_jual 	 = 0;
				...
				foreach ($cek_detail as $val_detail) {
					...
					$updateTotalHarga = array ( 'harga_jual'	=> $harga_jual,
												'total_hpp'		=> $hpp_jual,
												'total_jual'	=> $total_harga,
												'total_harga'	=> $total_harga,
												'selisih_margin'=> ($total_harga - $total_harga) - $ongkir - $hpp_jual,
												'margin'		=> $total_harga	- $total_harga
											);
	...
	public function sinkron_total_harga_sementara()
	{
		...
		$cek_pesanan = $this->Keluar_sementara_model->get_all_by_periodik_impor($start, $end);
