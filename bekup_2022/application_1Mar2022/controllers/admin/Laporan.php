...
class Laporan extends CI_Controller {
	...
	public function export_google_contacts($users, $periodik)
	{
		date_default_timezone_set("Asia/Jakarta");
		$now = date('Y-m-d H:i:s');
		$start = substr($periodik, 0, 10);
		...
		$users = $this->Auth_model->get_by_id($users);
		if ($users->usertype == 1 OR $users->usertype == 11) {
			$data['title']	= "Export Format Google Contacts Per Tanggal ".$start." - ".$end."_".date("H_i_s");
			$data['export'] = $this->Keluar_model->get_all_detail_by_periodik_google_contacts($start, $end);
