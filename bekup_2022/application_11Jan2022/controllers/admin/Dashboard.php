...
class Dashboard extends CI_Controller {
  ...
  //   	echo json_encode($msg);
	...
	public function ajax_dasbor_total()
	{
		...
		$max_pending  = max(array($get_income->total_pending, $get_income_past->total_pending));
		...
		if ($max_diterima == NULL && $max_income == NULL && $max_laba == NULL && $max_ongkir == NULL && $max_pesan == 0 && $max_pending == NULL) {
			...
			if ($max_pending == 0) {
				...
				$persen_pending	= (($get_income->total_pending - $get_income_past->total_pending) / $max_pending) * 100;
				$sisa_pending	= $get_income->total_pending - $get_income_past->total_pending;

				if ($persen_pending == 0 && $sisa_pending == 0) {
					$html_pending = '<span class="description-percentage text-yellow"><i class="fa fa-caret-left" style="margin-right:5px;"></i> 0% </span>'.
									 '<h5 class="description-header">'.rupiah($get_income->total_pending).'</h5>'.
		               	 			 '<span class="description-text">TOTAL PENDING PAYMENT</span>';
				}else if ($persen_pending < 0 && $sisa_pending < 0) {
					$html_pending = '<span class="description-percentage text-red"> -'.rupiah($sisa_pending * -1).' (<i class="fa fa-caret-down" style="margin-right:5px;"></i>'.round($persen_pending * -1).' %)</span>'.
									 '<h5 class="description-header">'.rupiah($get_income->total_pending).'</h5>'.
		               	 			 '<span class="description-text">TOTAL PENDING PAYMENT</span>';
				}else if ($persen_pending > 0 && $sisa_pending > 0) {
					$html_pending = '<span class="description-percentage text-green">+'.rupiah($sisa_pending).' (<i class="fa fa-caret-up" style="margin-right:5px;"></i>'.round($persen_pending).'%)</span>'.
									 '<h5 class="description-header">'.rupiah($get_income->total_pending).'</h5>'.
		               	 			 '<span class="description-text">TOTAL PENDING PAYMENT</span>';
				}
			}
