...
class Keluar extends CI_Controller {
	...
	public function __construct()
	{
	    ...
	    $this->load->model(array('Bahan_kemas_model', 'Vendor_model', 'Venmasaccess_model', 'Produk_model', 'Toko_model', 'Tokproaccess_model', 'Kurir_model', 'Keluar_model', 'Keluar_sementara_model', 'Paket_model', 'Resi_model', 'Dashboard_model', 'Status_transaksi_model', 'Keyword_model', 'Resi_model'));
	...
	public function dasbor_penjualan()
	{
		...
		$this->data['get_all_gudang_impor'] = $this->Keluar_model->get_all_gudang_list();
		$this->data['get_all_toko_penjualan'] = $this->Keluar_model->get_all_toko_list();
		$this->data['get_all_gudang_penjualan'] = $this->Keluar_model->get_all_gudang_list();
				...
				foreach ($storing as $val_store) {
					...
					if (count($cek_pesanan) >= $count_nomor_pesanan[$val_store['nomor_pesanan']]) {
						...
						if ($count_nomor_pesanan[$val_store['nomor_pesanan']] > count($cek_pesanan_sementara)) {
							...
							if (!isset($cek_kurir) OR !isset($cek_toko) OR !isset($cek_produk) OR !isset($cek_status_transaksi)) {
								$teks = '';
								if (!isset($cek_kurir)) {
