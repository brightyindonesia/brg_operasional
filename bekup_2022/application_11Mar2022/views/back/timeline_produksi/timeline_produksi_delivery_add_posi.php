...
<div class="wrapper">
            ...
            <div class="row">
              <div class="col-sm-6">
                <div class="form-group"><label>Start - End Date (*)</label>
                  <input type="text" class="form-control" value="<?php echo $detail_timeline->start_date_detail_timeline_produksi.' - '.$detail_timeline->end_date_detail_timeline_produksi ?>" id="range-date">
                </div>
              </div>
                      ...
                      <tbody>
                        <?php 
                          ...
                          foreach ($reject as $row_r) {
                            if ($row_r->fix_sisa_stok_pabrik != 0) {
                        ?>
                        <tr>
                          <td>
                            ...
                            <input type="hidden" id="dr_id_detail[]" name="dr_id_detail[]" value="<?php echo $row_r->id_stok_pabrik; ?>">
                          ...
                          <td>
                              <input type="hidden" id="dr_qty<?php echo $i ?>" name="dr_qty[]" value="<?php echo $row_r->qty_stok_pabrik ?>">
                              <?php echo $row_r->qty_stok_pabrik ?>
                          </td>
                          <td>
                            <input type="hidden" style="width: 110px;text-align: center;" type="text" name="dr_terpakai[]" id="dr_terpakai<?php echo $i ?>" value="<?php echo $row_r->terpakai_stok_pabrik ?>">
                              <?php echo $row_r->terpakai_stok_pabrik + $row_r->reject_stok_pabrik ?>

                          </td>
                          <td>
                            <input style="width: 110px;text-align: center;" type="hidden" name="dr_sisa[]" id="dr_sisa<?php echo $i ?>" value="<?php echo $row_r->fix_sisa_stok_pabrik ?>">
                            <!-- <input disabled style="width: 110px;text-align: center;" type="text" id="dr_sisa_terpakai<?php echo $i ?>" value="<?php echo $row_r->qty_detail_timeline_produksi ?>"> -->
                              <?php echo $row_r->fix_sisa_stok_pabrik ?>

                          </td>
                          <td>
                            <!-- <input type="text" style="width: 110px;text-align: center;" type="text" oninput="checkValue(this);calculate('dr_qty<?php echo $i ?>', 'dr_terpakai<?php echo $i ?>', 'dr_sisa<?php echo $i ?>', 'dr_sisa_fix<?php echo $i ?>', 'dr_selisih<?php echo $i ?>');" min="1" max="<?php echo $row_r->fix_sisa_stok_pabrik ?>" name="dr_sisa_fix[]" id="dr_sisa_fix<?php echo $i ?>" value="0"> -->
                            <input type="text" style="width: 110px;text-align: center;" type="text" oninput="checkValue(this);calculate('dr_sisa<?php echo $i ?>', 'dr_sisa_fix<?php echo $i ?>', 'dr_selisih<?php echo $i ?>', 'td_selisih<?php echo $i ?>');" min="1" max="<?php echo $row_r->fix_sisa_stok_pabrik ?>" name="dr_sisa_fix[]" id="dr_sisa_fix<?php echo $i ?>" value="<?php echo $row_r->fix_sisa_stok_pabrik ?>">
                          </td>
                          <td>
                            <input style="width: 110px;text-align: center;" type="hidden" name="dr_selisih[]" id="dr_selisih<?php echo $i ?>" value="0">
                            <div id="td_selisih<?php echo $i ?>">
                              0
                            </div>
                          </td>
                        </tr>
  ...
  <script type="text/javascript">
    ...
    function calculate(sisaId, sisafixId, selisihId, selisihTD)
    {
       sisa = document.getElementById(sisaId).value;
       sisa_fix = document.getElementById(sisafixId).value;
       selisih = document.getElementById(selisihId).value;
       ...
       document.getElementById(selisihTD).innerHTML = document.getElementById(selisihId).value;
    ...
    $('#bahan-delivery-add-posi').click(function(e){
      ...
      var dr_sisa =  $("input[name='dr_sisa[]']")
            .map(function(){return $(this).val();}).get();
      var dr_sisa_fix =  $("input[name='dr_sisa_fix[]']")
            .map(function(){return $(this).val();}).get();
      ...
      var JS_dr_sisa_fix = JSON.stringify(dr_sisa_fix);
      ...
      if (nomor_po == '' && nomor_produksi == '' && keterangan == '' && sku == '') {
        ...
        $.ajax({ 
          url:"<?php echo base_url()?>admin/timeline_produksi/proses_add_posi_delivery",
          method:"post",
          dataType: 'JSON',
          data:{id_detail: id_detail,nomor_po:nomor_po, nomor_produksi: nomor_produksi, keterangan: keterangan, date: date, qty: qty, sku: sku, dr_id: JS_dr_id, dr_id_detail: JS_dr_id_detail, dr_po: JS_dr_po, dr_qty: JS_dr_qty, dr_terpakai: JS_dr_terpakai, dr_sisa: JS_dr_sisa, dr_sisa_fix: JS_dr_sisa_fix, dr_selisih: JS_dr_selisih, length_dr: panjangArray_dr, [csrfName]: csrfHash},
