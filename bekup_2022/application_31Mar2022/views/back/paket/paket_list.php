...
<div class="wrapper">
            ...
            <table id="datatable" class="table table-bordered table-striped">
              ...
              <tbody>
                <?php $no = 1; foreach($get_all as $data){
                  // action
                  $edit = '<a href="'.base_url('admin/paket/ubah/'.$data->id_paket).'" class="btn btn-sm btn-warning"><i class="fa fa-pencil"></i></a>';
                  $delete = '<a href="'.base_url('admin/paket/hapus/'.$data->id_paket).'" onClick="return confirm(\'Are you sure?\');" class="btn btn-sm btn-danger"><i class="fa fa-trash"></i></a>';
                ?>
                  <tr>
                    <td style="text-align: center"><?php echo $no++ ?></td>
                    <td style="text-align: center"><?php echo $data->nama_paket ?></td>
                    <td style="text-align: center"><?php echo $edit ?> <?php echo $delete ?></td>
                    <td style="text-align: center">
                      <input type="checkbox" class="sub_chk" data-id="<?php echo $data->id_paket ?>">
                    </td>
                  </tr>
                <?php } ?>
              </tbody>
  ...
  $(document).ready( function () {
    ...
    $('#btn-ekspor').click(function(){
      ...
      if(check == true){
        window.open("<?php echo base_url() ?>admin/paket/export_paket");
      }
    });
    ...
    $('#btn-delete-pilih').on('click', function(e) {
          ...
          {  
              ...
              if(check == true){  
                  ...
                  $.ajax({
                      url: "<?php echo base_url()?>admin/paket/hapus_dipilih",
                      type: 'POST',
                      dataType: 'JSON', 
