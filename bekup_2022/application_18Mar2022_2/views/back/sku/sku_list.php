...
<div class="wrapper">
            ...
            <table id="datatable" class="table table-bordered table-striped">
              ...
              <tbody>
                <?php $no = 1; foreach($get_all as $data){
                  // action
                  $edit = '<a href="'.base_url('admin/sku/ubah/'.$data->id_sku).'" class="btn btn-sm btn-warning"><i class="fa fa-pencil"></i></a>';
                  $delete = '<a href="'.base_url('admin/sku/hapus/'.$data->id_sku).'" onClick="return confirm(\'Are you sure?\');" class="btn btn-sm btn-danger"><i class="fa fa-trash"></i></a>';
                ?>
                  <tr>
                    ...
                    <td style="text-align: center">
                      <input type="checkbox" class="sub_chk" data-id="<?php echo $data->id_sku ?>">
                    </td>
                  </tr>
                <?php } ?>
              </tbody>
  ...
  $(document).ready( function () {
    ...
    $('#btn-ekspor').click(function(){
      ...
      if(check == true){
        window.open("<?php echo base_url() ?>admin/sku/export_sku");
      }
    });
    ...
    $('#btn-delete-pilih').on('click', function(e) {
          ...
          {  
              ...
              if(check == true){  
                  ...
                  $.ajax({
                      url: "<?php echo base_url()?>admin/sku/hapus_dipilih",
                      type: 'POST',
                      dataType: 'JSON', 
                      ...
                      success: function (data) {
                        ...
                        if (data.sukses) {
                          Toast.fire({
                            ...
                            window.location.replace("<?php echo base_url()?>admin/sku");
                          });
                        }
                        // console.log(data);
                        // $(".sub_chk:checked").each(function() {  
