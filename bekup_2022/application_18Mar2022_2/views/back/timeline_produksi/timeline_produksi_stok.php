            ...
            <div class="row">
              ...
              <div class="col-sm-6">
                  ...
                  <div class="form-group"><label>Tanggal (*)</label>
                    <input type="text" name="periodik" class="form-control float-right" id="tgl-now">
                  </div>
              </div>
            </div>
  ...
  <script type="text/javascript">
    ...
    $('#po').on('change', function(){
      ...
      if (po != '') {
        ...
        $.ajax({
          url: "<?php echo base_url()?>admin/timeline_produksi/get_no_po_stok",
          type: "post",
    ...
    $('#bahan').on('change', function(){
      ...
      if (bahan != '' && nomor_po != '') {
        $.ajax({
          url: "<?php echo base_url()?>admin/timeline_produksi/get_id_bahan_stok",
          type: "post",
          ...
          success: function(data){
            ...
            document.getElementById("in_qty").value = data.selisih_po_produksi;
            $("#in_qty").attr("max", data.selisih_po_produksi)
            $("#in_qty").attr("max", 1)
            document.getElementById("in_harga").value = data.harga_po;
            document.getElementById("in_stok").value = data.selisih_po_produksi;
            document.getElementById("in_jumlah").value = data.harga_po * data.selisih_po_produksi;
    ...
    $('#add_bahan').click(function(){
      ...
      if (id_bahan == '' || nama_bahan == '' || po == '' || qty == '' || harga == '' || jumlah == '') {
        Toast.fire({
          icon: 'error',
          title: 'Perhatian!',
    ...
    $('#bahan-industry').click(function(e){
      ...
      var date = document.getElementById('tgl-now').value;
      ...
      csrfHash = '<?php echo $this->security->get_csrf_hash(); ?>';
      // alert(panjangArray);
      if (nomor_po == '' && nomor_produksi == '' && keterangan == '' && sku == '' && dt_id == '' && dt_po == '' && dt_qty == '' && dt_harga == '' && dt_jumlah == '') {
        Toast.fire({
          icon: 'error',
          title: 'Terjadi Kesalahan!. Harap diisi!'
        });
      }else if(dt_id == '' || dt_po == '' || dt_qty == '' || dt_harga == '' || dt_jumlah == ''){
        Toast.fire({
          icon: 'error',
        ...
        $.ajax({ 
          url:"<?php echo base_url()?>admin/timeline_produksi/stok_pabrik_proses",
          method:"post",
          dataType: 'JSON', 
          data:{nomor_po:nomor_po, nomor_produksi: nomor_produksi,qty: qty, sku: sku, date: date, dt_id: JS_id, dt_po: JS_po, dt_qty: JS_qty, dt_harga: JS_harga, dt_jumlah: JS_jumlah, length: panjangArray, [csrfName]: csrfHash},
          success:function(data)  {  
            // alert(data);
            if (data.validasi) {
              Toast.fire({
            ...
            if (data.sukses) {
              Toast.fire({
                ...
                window.location.replace("<?php echo base_url()?>admin/timeline_produksi/history/"+data.nopro);
              });
            }
