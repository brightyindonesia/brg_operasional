...
class Keyword_model extends CI_Model {
  ...
  function get_all_provinsi_combobox_list()
  {
    $this->db->order_by('nama_provinsi');
    $data = $this->db->get($this->table_provinsi);
    ...
    if($data->num_rows() > 0)
    {
      foreach($data->result_array() as $row)
      {
        ...
        $result[$row['nama_provinsi']] = $row['nama_provinsi'];
      }
      return $result;
    }
  }

  function get_all_provinsi_combobox()
  {
    $this->db->order_by('nama_provinsi');
    $data = $this->db->get($this->table_provinsi);
  ...
  function get_nama_provinsi_by_provinsi($provinsi)
  {
    if ($provinsi != 'semua') {
      $query = $this->db->get_where($this->table_provinsi, array('nama_provinsi' => $provinsi));
    }else{
  ...
  function get_nama_kotkab_by_kotkab($kotkab)
  {
    if ($kotkab != 'semua') {
      $query = $this->db->get_where($this->table_detail_provinsi, array('nama_kotkab' => $kotkab));
    }else{
      $query = $this->db->get($this->table_detail_provinsi);
    }    
    return $query->row();    
  }

  public function get_kabupaten_by_provinsi($provinsi)
  {
    $this->db->join($this->table_provinsi, 'detail_keyword_provinsi.id_keyword_provinsi = keyword_provinsi.id_keyword_provinsi');
    $this->db->group_by('nama_kotkab');
    $query = $this->db->get_where($this->table_detail_provinsi, array('nama_provinsi' => $provinsi));
  ...
  function get_kotkab_provinsi_by_keys_kotkab($id)
  {
  ...
  function get_produk_by_keys_produk($id)
  {
    $this->db->join($this->table_produk, 'detail_keyword_produk.id_keyword_produk = keyword_produk.id_keyword_produk');
    $this->db->select('*');
    $this->db->where('keys_produk', $id);
  ...
  function get_toko_by_keys_toko($id)
  {
    $this->db->join($this->table_toko, 'detail_keyword_toko.id_keyword_toko = keyword_toko.id_keyword_toko');
    $this->db->select('*');
    $this->db->where('keys_toko', $id);
  ...
  function get_kurir_by_keys_kurir($id)
  {
    $this->db->join($this->table_kurir, 'detail_keyword_kurir.id_keyword_kurir = keyword_kurir.id_keyword_kurir');
    $this->db->select('*');
    $this->db->where('keys_kurir', $id);
