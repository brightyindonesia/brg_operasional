...
class Surat_model extends CI_Model
{
  ...
  public $table_terima_barang = 'surat_terima_barang';
  public $id_terima_barang     = 'nomor_surat_terima_barang ';

  public $table_detail_terima_barang  = 'detail_surat_terima_barang';
  public $id_detail_terima_barang   = 'no_surat_terima_barang ';
  ...
  public $column_order_terima_barang = array(null, 'tgl_terima_surat_terima_barang', 'nomor_surat_terima_barang', 'nama_surat_terima_barang', null); //field yang ada di table user
  public $column_search_terima_barang = array('	tgl_terima_surat_terima_barang', 'no_surat_jalan', 'no_po', 'nama_penerima_surat_terima_barang'); //field yang diizin untuk pencarian 
  public $order_data_terima_barang = array('tgl_terima_surat_terima_barang' => 'asc'); // default order 
  // Tabel Server Side
  // Surat Jalan
  private function _get_datatables_query_surat_jalan()
  {
    ...
    // ));
    $this->db->where(array(
      "date_format(created_surat_jalan, '%Y-%m-%d') >="   => $start,
      "date_format(created_surat_jalan, '%Y-%m-%d') <="   => $end
    ));
  }
  ...
  private function _get_datatables_query_surat_terima_barang()
  {
    $i = 0;

    foreach ($this->column_search_terima_barang as $item) // looping awal
    {
      if ($_GET['search']['value']) // jika datatable mengirimkan pencarian dengan metode POST
      {
        ...
        if (count($this->column_search_terima_barang) - 1 == $i)
          $this->db->group_end();
      }
      $i++;
    }
    ...
    if (isset($_GET['order'])) {
      $this->db->order_by($this->column_order_terima_barang[$_GET['order']['0']['column']], $_GET['order']['0']['dir']);
    } else if (isset($this->order_data_terima_barang)) {
      $order = $this->order_data_terima_barang;
      $this->db->order_by(key($order), $order[key($order)]);
    }

    $start = substr($_GET['periodik'], 0, 10);
    $end = substr($_GET['periodik'], 13, 24);
    $this->db->order_by('tgl_terima_surat_terima_barang', 'desc');
    $this->db->from($this->table_terima_barang);

    if ($_GET['warehouse'] != 'semua') {
      $this->db->where('id_warehouse_surat_terima_barang', $_GET['warehouse']);
    }
    // $this->db->where( array(  "tgl_penjualan >="   => $first,
    //                           "tgl_penjualan <="   => $last
    // ));
    $this->db->where(array(
      "date_format(tgl_terima_surat_terima_barang, '%Y-%m-%d') >="   => $start,
      "date_format(tgl_terima_surat_terima_barang, '%Y-%m-%d') <="   => $end
    ));
  }


  function get_datatables_surat_terima_barang()
  {
    $this->_get_datatables_query_surat_terima_barang();
    if ($_GET['length'] != -1)
      $this->db->limit($_GET['length'], $_GET['start']);
    $query = $this->db->get();
    return $query->result();
  ...
  function count_filtered_surat_terima_barang()
  {
    $this->_get_datatables_query_surat_terima_barang();
    $query = $this->db->get();
    return $query->num_rows();
  ...
  public function count_all_surat_terima_barang()
  {
    $this->db->from($this->table_terima_barang);
    return $this->db->count_all_results();
  }
  ...
  function get_dasbor_list($penerima, $first, $last)
  {
    ...
    //                         ));
    $this->db->where(array(
      "date_format(created_surat_jalan, '%Y-%m-%d') >="   => $first,
  ...
  function get_detail_surat_jalan_by_id_row($id)
  {
    $this->db->order_by($this->id_detail, 'asc');
  ...
  {
    $this->db->order_by('created_surat_jalan', $this->order);
  ...
  private function _get_datatables_query_surat_packing()
  {
    ...
    // ));
    $this->db->where(array(
      "date_format(created_surat_packing, '%Y-%m-%d') >="   => $start,
      "date_format(created_surat_packing, '%Y-%m-%d') <="   => $end
    ));
  }
  ...
  function get_dasbor_list_packing($penerima, $first, $last)
  {
    ...
    //                         ));
    $this->db->where(array(
      "date_format(created_surat_packing, '%Y-%m-%d') >="   => $first,
  ...
  function get_detail_surat_packing_by_id_row($id)
  {
    $this->db->order_by($this->id_detail_packing, 'asc');
    $this->db->where($this->id_detail_packing, $id);
    return $this->db->get($this->table_detail_packing)->row();
  }

  function cari_nomor_sp($nomor)
  {
    $this->db->order_by('created_surat_packing', $this->order);
  ...
  function delete_detail_by_id_packing($id)
  {
    $this->db->where($this->id_detail_packing, $id);
    $this->db->delete($this->table_detail_packing);
  }
  ...
  function count_total_surat_terima_barang()
  {
    return $this->db->from($this->table_terima_barang)->count_all_results();
  }

  function insert_terima($data)
  {
    $this->db->insert($this->table_terima_barang, $data);
  }

  function insert_detail_terima($data)
  {
    $this->db->insert($this->table_detail_terima_barang, $data);
  }

  function get_surat_terima_barang_by_nomor($id)
  {
    $this->db->where($this->id_terima_barang, $id);
    return $this->db->get($this->table_terima_barang)->row();
  }

  function get_detail_surat_terima_barang_by_nomor($id)
  {
    $this->db->order_by($this->id_detail_terima_barang, 'asc');
    $this->db->join('sku', 'sku.id_sku = '.$this->table_detail_terima_barang. '.nama_barang_surat_jalan');
    $this->db->where($this->id_detail_terima_barang, $id);
    return $this->db->get($this->table_detail_terima_barang)->result();
  }

  function delete_surat_terima_detail_by_nomor($id)
  {
    $this->db->where($this->id_detail_terima_barang, $id);
    $this->db->delete($this->table_detail_terima_barang);
  }

  function delete_surat_terima($id)
  {
    $this->db->where($this->id_terima_barang, $id);
    $this->db->delete($this->table_terima_barang);
  }

  function get_all_penerima_surat_terima_list()
  {
