...
<div class="wrapper">
    ...
    <div class="box box-primary">
        <div class="box box-header">
          <div class="form-group"><label>Pilih Tanggal</label>
            <input type="text" name="periodik" class="form-control float-right" id="range-date" <?= isset($_GET['periodik']) ? "value='$_GET[periodik]'" : '' ?> >
          </div>
        </div>
      </div>
      ...
      <div class="row">
        <div class="col-sm-12">
          <?php $this->load->view('back/keluar/tab_content/content_dashboard_insight.php') ?> 
        </div>
      </div>
    </section>
    <!-- /.content -->
  ...
  <script>
    ...
    $(document).ready(function() {
      $('#range-date').on('change', function () {
        window.open('<?php echo base_url('admin/keluar/dashboard_customer_insight?periodik=') ?>'+$(this).val(), '_self');
      });
    });
  </script>
  <!-- <script type="text/javascript">
    ...
    function tabelDataRepeat(hp, first, last, trigger) {
      if ($.fn.DataTable.isDataTable("#table-modal-repat-order")) {
        $('#table-modal-repat-order').DataTable().clear().destroy();
        $('#table-modal-repat-order').dataTable().fnDestroy();
      }

      document.getElementById("judul-total-repeat").innerHTML = "Daftar Pesanan";
      document.getElementById("ekspor-repeat").innerHTML = '<a href="javascript:void(0)" onclick="exportRepeat(\''+hp+'\',\''+first+'\',\''+last+'\',\''+trigger+'\')" class="btn btn-sm btn-success"><i class="fa fa-file-excel-o"></i> Export Data</a>';

      $('#modal-total-repeat').modal('show');
      ...
      var table = $('#table-modal-repat-order').DataTable({
          "iDisplayLength":10,
          "processing": false,
          "serverSide": true,
          "autoWidth": false,
          'ajax': {
              'url': '<?php echo base_url()?>admin/dashboard/get_data_modal_data_repeat',
              'data': function(d){
                d.first = first;
                d.last = last;
                d.hp = hp;
      ...
      // Add event listener for opening and closing details
      $('#table-modal-repat-order').off('click').on('click', 'td.details-control', function () {
          var tr = $(this).closest('tr');
          var row = table.row(tr);
          // var data = table.row($(this).parents('tr')).data();
          var data = table.row($(this).parents('tr')).data();
    ...
    function exportRepeat(hp, first, last, trigger) {
      if (hp == null || hp == '') {
        window.open("<?php echo base_url() ?>admin/dashboard/export_repeat_modal_penjualan/kosong/"+first+"/"+last+"/"+trigger,+"_self");
      }else{
        window.open("<?php echo base_url() ?>admin/dashboard/export_repeat_modal_penjualan/"+hp+"/"+first+"/"+last+"/"+trigger,+"_self");
      }
      ...
      function dasbor_total() {
        ...
        $.ajax({
              ...
              success:function(data)  {  
                ...
                document.getElementById("dasbor-total-pending").innerHTML= data.pending;
                document.getElementById("dasbor-total-revenue").innerHTML= data.revenue;
                document.getElementById("dasbor-total-bruto").innerHTML= data.bruto;
                document.getElementById("dasbor-total-gross").innerHTML=data.gross;
                document.getElementById("dasbor-total-ongkir").innerHTML=data.ongkir;
                document.getElementById("dasbor-total-margin").innerHTML=data.margin;
                document.getElementById("dasbor-total-selisih-margin").innerHTML=data.selisih_margin;
                document.getElementById("dasbor-total-hpp").innerHTML=data.hpp;
              },
              error: function(data){
                console.log(data.responseText);
              }
      ...
      function dasbor_total_penjualan() {
        ...
        $.ajax({
              ...
              success:function(data)  {  
                ...
                document.getElementById("dasbor-total-pending-penjualan").innerHTML= data.pending;
                document.getElementById("dasbor-total-revenue-penjualan").innerHTML= data.revenue;
                document.getElementById("dasbor-total-bruto-penjualan").innerHTML= data.bruto;
                document.getElementById("dasbor-total-gross-penjualan").innerHTML=data.gross;
                document.getElementById("dasbor-total-ongkir-penjualan").innerHTML=data.ongkir;
                document.getElementById("dasbor-total-margin-penjualan").innerHTML=data.margin;
                document.getElementById("dasbor-total-selisih-margin-penjualan").innerHTML=data.selisih_margin;
                document.getElementById("dasbor-total-hpp-penjualan").innerHTML=data.hpp;
              },
              error: function(data){
                console.log(data.responseText);
              }
