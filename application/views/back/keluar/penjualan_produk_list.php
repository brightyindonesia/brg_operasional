...
<div class="wrapper">
            ...
            <div class="box-header">
                ...
                <div class="col-sm-12">
                  <div class="form-group">
                    <label style="margin-right: 15px;">
                      <input type="radio" name="jenis_diterima" id="jenis-diterima" value="transfer" checked>
                      Transfer
                    </label>

                    <label>
                      <input type="radio" name="jenis_diterima" id="jenis-diterima" value="pending">
                      Pending Payments
                    </label>
                  </div>
                </div>
              ...
              <div class="row">
                <div class="col-sm-12">
                  ...
                  <div class="form-group">
                    <button id="btn-diterima" name="button" class="btn btn-success"><i class="fa fa-upload"></i> Import</button>
  ...
  // ============= PROSES PEMBAYARAN DITERIMA (TF / PENDING PAYMENTS) ====================
  $('#btn-diterima').click(function(e){
      ...
      var imports = document.getElementById('impor_diterima');
      ...
      if(imports.files['length'] == 0){
        ...
        formData.append('impor_diterima', $('#impor_diterima')[0].files[0]);  
        // formData.append('keyword', $('#filter-keyword').val());      
        formData.append('jenis', $('input[type=radio][name=jenis_diterima]:checked').attr('value'));      
        formData.append([csrfName], csrfHash); 

        $("#modal-proses").modal('show');
        $.ajax({ 
          url:"<?php echo base_url()?>admin/keluar/proses_impor_diterima_new",
          method:"post",
          dataType: 'JSON', 
          ...
          success:function(data)  {  
            ...
            document.getElementById("impor_diterima").value = null;

            if (data.validasi) {
              $("#modal-proses").modal('hide');
              if (data.pesan_error) {
                $('#message-validasi-diterima').html('<div class="alert alert-danger">'+data.pesan_error+'</div>');
              }
              document.getElementById("impor_diterima").value = null;
              Toast.fire({
                icon: 'error',
