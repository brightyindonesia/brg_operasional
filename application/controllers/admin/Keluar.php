...
class Keluar extends CI_Controller
{
			...
			foreach ($get_produk as $val_pro) {
				...
				if (isset($cek_propak)) {
					foreach ($cek_propak as $val_propak) {
						...
						$produk_data[$i]['data'][] = array(
							0	=> $val_propak->nama_produk,
							1	=> $int_fix
						);
					}
				} else {
	...
	public function ajax_dasbor_prokur_2()
	{
		...
		foreach ($get_toko as $val_tok) {
			...
			foreach ($get_kurir as $val_kur) {
				...
				$kurir_data[$i]['data'][] = array(
					0	=> $val_kur->nama_kurir,
					1	=> $int_jumlah_kur
				);
			}
	...
	public function migrate_data_sementara()
	{
		...
		if (count($ambil_data) > 0) {
				...
				if (isset($cek_nomor)) {
					...
					if (count($cek_detail) > 0) {
						foreach ($cek_detail as $val_detail) {
							$simpanDetail		= array(
								'nomor_pesanan'		=> $val_detail->nomor_pesanan,
							...
							if (isset($cariPaket)) {
								...
								if (count($produkPaket) > 0) {
									...
									foreach ($produkPaket as $result) {
										...
										$kurangStokPakduk = array(
											'qty_produk' 		=> $result->qty_produk - $total
										);
							...
							$kurangStokProduk 		= array(
								'qty_produk' 		=> $cariProduk->qty_produk - $val_detail->qty
							);
					...
					$simpanPenjualan	= array(
						'nomor_pesanan'			=> $val_sementara->nomor_pesanan,
			...
			$msg = array(
				'sukses'	=> $baris . ' Data Berhasil Import Penjualan!. ' . $resi . ' Data Resi ditambah!',
			);
			echo json_encode($msg);
		} else {
			$msg = array(
	...
	public function ubah_proses()
	{
		...
		if (isset($cek_no_pesanan)) {
			...
			foreach ($cariDetail as $produk) {
				...
				// END
				$nambahStok = array(
					'qty_produk' 		=> $produk->qty_produk + $produk->qty
				);
			...
			for ($n = 0; $n < $len; $n++) {
				...
				// END
				$kurangStokProduk[$n] = array(
					'qty_produk' 		=> $cariProduk[$n]->qty_produk - $decode_qty[$n]
				);
				...
				for ($n = 0; $n < $len; $n++) {
					...
					if (isset($cariPaket[$n])) {
						...
						if (count($produkPaket) > 1) {
							...
							$kurangStokPakduk = array(
								'qty_produk' 		=> $rowPaket->qty_produk - $total
							);
	...
	public function penjualan_hapus_dipilih()
	{
		...
		if (isset($cekDetail)) {
			foreach ($cekDetail as $detail) {
				...
				$tambahStok = array(
					'qty_produk' 		=> $row_produk->qty_produk + $detail->qty
				);
		...
		if ($keyword == "no") {
			...
			if ($this->upload->do_upload('impor_penjualan')) {
					...
					if ($numSheet == 0) {
						foreach ($sheetXLS->getRowIterator() as $row) {
							...
							if ($numRow > 1) {
								...
								$cells 	   = $row->getCells();
								$storing[] = array(
									'nomor_pesanan'			=> $cells[0]->getValue(),
					...
					if (count($cek_pesanan) >= $count_nomor_pesanan[$val_store['nomor_pesanan']]) {
						...
						if ($count_nomor_pesanan[$val_store['nomor_pesanan']] > count($cek_pesanan_sementara)) {
							...
							if (!isset($cek_kurir) or !isset($cek_toko) or !isset($cek_produk) or !isset($cek_status_transaksi) or !isset($cek_provinsi) or !isset($cek_kotkab)) {
								...
								$storing_fix[] = array(
									'nomor_pesanan'			=> $val_store['nomor_pesanan'],
						...
						if ($count_nomor_pesanan[$val_store['nomor_pesanan']] > count($cek_pesanan)) {
							...
							foreach ($cek_pesanan as $val_pesanan) {
								...
								$tambahStok = array(
									'qty_produk' 		=> $row_produk->qty_produk + $val_pesanan->qty
								);
				...
				if (count($storing_fix) > 0) {
					...
					foreach ($storing_fix as $val_fix) {
						...
						if (isset($cek_nomor)) {
							...
							if (isset($cek_pesanan_produk)) {
								...
								$updateDetail		= array(
									'qty' 				=> ((int)$cek_pesanan_produk->qty + (int)$val_fix['qty']),
								);
								...
								$simpanDetail		= array(
									'nomor_pesanan'		=> $val_fix['nomor_pesanan'],
					...
					if ($validasi > 0) {
						$msg = array(
							'sukses'	=> $baris . ' Data Berhasil Import Penjualan ke Data Sementara!',
							'pesan_error'	=> $msg_err
						);
						echo json_encode($msg);
					} else {
					...
					foreach ($sheet->getRowIterator() as $row) {
						if ($numRow > 1) {
							...
							if (isset($cek_nomor)) {
								...
								$simpanDetail		= array(
									'nomor_pesanan'		=> $row->getCellAtIndex(0),
								...
								$kurangStokProduk 		= array(
									'qty_produk' 		=> $cariProduk->qty_produk - $qty
								);
		...
		if ($this->input->post('jenis') == 'pending') {
			...
			if ($this->upload->do_upload('impor_diterima')) {
				...
				if ($baris > 0) {
					if ($validasi > 0) {
						$msg = array(
							'sukses'	=> $baris . ' Data payment successfully!',
							'pesan_error'	=> $msg_err
						);
						echo json_encode($msg);
					} else {
					...
					if ($validasi > 0) {
						$msg = array(
							'validasi'	=> 'No data payment successfully!',
							'pesan_error'	=> $msg_err
						);
						echo json_encode($msg);
					} else {
	...
	public function dashboard_customer_insight()
	{
		...
		$this->data['jumlah_pembeli'] = $this->input->get('periodik') ? $this->db->query("SELECT DISTINCT REPLACE(REPLACE(REPLACE(hp_penerima, '0', ''), '62', ''), ' ', '') FROM penjualan WHERE date_format(tgl_penjualan, '%Y-%m-%d') >= ? AND date_format(tgl_penjualan, '%Y-%m-%d') <= ?", array($start, $end))->num_rows() : $this->db->query("SELECT DISTINCT REPLACE(REPLACE(REPLACE(hp_penerima, '0', ''), '62', ''), ' ', '') FROM penjualan WHERE date_format(tgl_penjualan, '%Y-%m') = ?", array(date('Y-m')))->num_rows();
		$this->data['pembeli_repeat_order'] = $this->input->get('periodik') ? $this->db->query("SELECT REPLACE(REPLACE(REPLACE(hp_penerima, '0', ''), '62', ''), ' ', ''), COUNT(REPLACE(REPLACE(REPLACE(hp_penerima, '0', ''), '62', ''), ' ', '')) as repeat_ord FROM penjualan WHERE date_format(tgl_penjualan, '%Y-%m-%d') >= ? AND date_format(tgl_penjualan, '%Y-%m-%d') <= ? GROUP BY REPLACE(REPLACE(REPLACE(hp_penerima, '0', ''), '62', ''), ' ', '') HAVING repeat_ord > 1", array($start, $end))->num_rows() : $this->db->query("SELECT REPLACE(REPLACE(REPLACE(hp_penerima, '0', ''), '62', ''), ' ', ''), COUNT(REPLACE(REPLACE(REPLACE(hp_penerima, '0', ''), '62', ''), ' ', '')) as repeat_ord FROM penjualan WHERE date_format(tgl_penjualan, '%Y-%m') = ? GROUP BY REPLACE(REPLACE(REPLACE(hp_penerima, '0', ''), '62', ''), ' ', '') HAVING repeat_ord > 1", array(date('Y-m')))->num_rows();
		$this->data['pembeli_baru'] = $this->input->get('periodik') ? $this->db->query("SELECT REPLACE(REPLACE(REPLACE(hp_penerima, '0', ''), '62', ''), ' ', ''), COUNT(REPLACE(REPLACE(REPLACE(hp_penerima, '0', ''), '62', ''), ' ', '')) as repeat_ord FROM penjualan WHERE date_format(tgl_penjualan, '%Y-%m-%d') >= ? AND date_format(tgl_penjualan, '%Y-%m-%d') <= ? GROUP BY REPLACE(REPLACE(REPLACE(hp_penerima, '0', ''), '62', ''), ' ', '') HAVING repeat_ord = 1", array($start, $end))->num_rows() : $this->db->query("SELECT REPLACE(REPLACE(REPLACE(hp_penerima, '0', ''), '62', ''), ' ', ''), COUNT(REPLACE(REPLACE(REPLACE(hp_penerima, '0', ''), '62', ''), ' ', '')) as repeat_ord FROM penjualan WHERE date_format(tgl_penjualan, '%Y-%m') = ? GROUP BY REPLACE(REPLACE(REPLACE(hp_penerima, '0', ''), '62', ''), ' ', '') HAVING repeat_ord = 1", array(date('Y-m')))->num_rows();
		// die(print_r($this->data['pembeli_baru']));
		if ($this->data['pembeli_repeat_order'] > 0 && $this->data['jumlah_pembeli'] > 0) {
			$this->data['repeat_order'] = ($this->data['pembeli_repeat_order'] / $this->data['jumlah_pembeli']) * 100;
		} else {
