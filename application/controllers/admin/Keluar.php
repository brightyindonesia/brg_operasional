...
class Keluar extends CI_Controller {
	...
	public function dashboard_customer_insight() {
		...
		$this->data['jumlah_pembeli'] = $this->input->get('periodik') ? $this->db->query("SELECT DISTINCT REPLACE(REPLACE(REPLACE(hp_penerima, '0', ''), '62', ''), ' ', '') FROM penjualan WHERE date_format(tgl_penjualan, '%Y-%m-%d') >= ? AND date_format(tgl_penjualan, '%Y-%m-%d') <= ?", array($start, $end))->num_rows() : $this->db->query("SELECT DISTINCT hp_penerima FROM penjualan WHERE date_format(tgl_penjualan, '%Y-%m') = ?", array(date('Y-m')))->num_rows();
		$this->data['pembeli_repeat_order'] = $this->input->get('periodik') ? $this->db->query("SELECT REPLACE(REPLACE(REPLACE(hp_penerima, '0', ''), '62', ''), ' ', ''), COUNT(REPLACE(REPLACE(REPLACE(hp_penerima, '0', ''), '62', ''), ' ', '')) as repeat_ord FROM penjualan WHERE date_format(tgl_penjualan, '%Y-%m-%d') >= ? AND date_format(tgl_penjualan, '%Y-%m-%d') <= ? GROUP BY REPLACE(REPLACE(REPLACE(hp_penerima, '0', ''), '62', ''), ' ', '') HAVING repeat_ord > 1", array($start, $end))->num_rows() : $this->db->query("SELECT REPLACE(REPLACE(REPLACE(hp_penerima, '0', ''), '62', ''), ' ', '') FROM penjualan WHERE date_format(tgl_penjualan, '%Y-%m') = ? GROUP BY hp_penerima HAVING COUNT(COUNT(DISTINCT REPLACE(REPLACE(REPLACE(hp_penerima, '0', '')) > 1", array(date('Y-m')))->num_rows();
		$this->data['pembeli_baru'] = $this->input->get('periodik') ? $this->db->query("SELECT REPLACE(REPLACE(REPLACE(hp_penerima, '0', ''), '62', ''), ' ', ''), COUNT(REPLACE(REPLACE(REPLACE(hp_penerima, '0', ''), '62', ''), ' ', '')) as repeat_ord FROM penjualan WHERE date_format(tgl_penjualan, '%Y-%m-%d') >= ? AND date_format(tgl_penjualan, '%Y-%m-%d') <= ? GROUP BY REPLACE(REPLACE(REPLACE(hp_penerima, '0', ''), '62', ''), ' ', '') HAVING repeat_ord = 1", array($start, $end))->num_rows() : $this->db->query("SELECT COUNT(DISTINCT COUNT(DISTINCT REPLACE(REPLACE(REPLACE(hp_penerima, '0', '')) as pembeli_baru FROM penjualan WHERE date_format(tgl_penjualan, '%Y-%m') = ? AND COUNT(DISTINCT REPLACE(REPLACE(REPLACE(hp_penerima, '0', '') IN (SELECT COUNT(DISTINCT REPLACE(REPLACE(REPLACE(hp_penerima, '0', '') FROM penjualan GROUP BY hp_penerima HAVING COUNT(*)=1)", array(date('Y-m')))->row();
