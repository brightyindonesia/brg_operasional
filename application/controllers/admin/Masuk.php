		...
		if ($keyword == "no") {
			...
			if ($this->upload->do_upload('impor_rfq')) {
					...
					if ($numSheet == 0) {
						foreach ($sheetXLS->getRowIterator() as $row) {
							...
							if ($numRow > 1) {
								...
								$storing[] = array(	'no_request'			=> $cells[0]->getValue(),
					...
					if (count($cek_request) >= $count_nomor_request[$val_store['no_request']]) {
						...
						if ($count_nomor_request[$val_store['no_request']] > count($cek_request)) {
							...
							if (!isset($cek_kategori) OR !isset($cek_vendor) OR !isset($cek_penerima) OR !isset($cek_sku) OR !isset($cek_bahan)) {
								...
								$storing_fix[] = array(	'no_request'			=> $val_store['no_request'],
				...
				if (count($storing_fix) > 0) {
						...
						if (isset($cek_nomor)) {
							...
							if (isset($cek_rfq_bahan)) {
								...
								if ($cek_rfq_bahan->ongkir == 0 AND $ongkir == 0 ){
									...
									$ubahRFQ 	= array(	'ongkir'			=> $ongkir_fix,
															'total_diskon' 		=> $total_diskon,
															'total_pajak' 		=> $total_pajak,
															'total_harga' 		=> $total_harga_fix,
														);
								...
								$updateDetail		= array(	'kuantitas_request' => ((int)$cek_rfq_bahan->kuantitas_request + (int)$val_fix['kuantitas_request'])
														);
								...
								$simpanDetailRFQ		= array(	'no_request'		=> $val_fix['no_request'],
							...
							$simpanRFQUtama		= array(	'no_request'			=> $val_fix['no_request'],
					...
					// Foreach
					if ($validasi > 0) {
						$msg = array(	'sukses'	=> $baris.' Data Berhasil Import RFQ ke Data RFQ!',
										'pesan_error'	=> $msg_err
			    			);
				    	echo json_encode($msg);	
					}else{
		...
		if ($keyword == "no") {
			...
			if ($this->upload->do_upload('impor_po')) {
					...
					if ($numSheet == 1) {
						foreach ($sheetXLS->getRowIterator() as $row) {
							...
							if ($numRow > 1) {
								...
								$cells 	   = $row->getCells();
								$tgl_fix   = $cells[1]->getValue();
								$storing[] = array(	'no_po'			=> $cells[0]->getValue(),
					...
					if (count($cek_po) >= $count_nomor_po[$val_store['no_po']]) {
						...
						if ($count_nomor_po[$val_store['no_po']] > count($cek_po)) {
							...
							if (!isset($cek_kategori) OR !isset($cek_vendor) OR !isset($cek_penerima) OR !isset($cek_sku) OR !isset($cek_bahan)) {
								...
								$storing_fix[] = array(	'no_po'			=> $val_store['no_po'],
				...
				if (count($storing_fix) > 0) {
						...
						if (isset($cek_nomor)) {
							...
							if (isset($cek_po_bahan)) {
								...
								if ($cek_po_bahan->ongkir_po == 0 AND $ongkir == 0 ){
									...
									$ubahPO	= array(	'ongkir_po'					=> $ongkir_fix,
														'total_diskon_po' 			=> $total_diskon,
														'total_pajak_po' 			=> $total_pajak,
														'total_harga_po' 			=> $total_harga_fix,
														'total_kuantitas_po'		=> $total_kuantitas,
								...
								$updateDetail		= array(	'kuantitas_po' => ((int)$cek_po_bahan->kuantitas_po + (int)$val_fix['kuantitas_po'])
														);
								...
								$simpanDetailPO		= array(	'no_po'					=> $val_fix['no_po'],
																'id_bahan_kemas'		=> $val_fix['id_bahan_kemas'],
							...
							$total_diskon 	= (($diskon * 0.01) * ($harga * $kuantitas));
							$total_pajak	= (($pajak * 0.01) * ($harga * $kuantitas));
							$simpanRFQUtama		= array(	'no_po'						=> $val_fix['no_po'],
															'id_kategori_po' 			=> $val_fix['id_kategori_po'],
															'id_users' 					=> $this->session->userdata('id_users'),
							...
							$simpanDetailRFQ		= array(	'no_po'					=> $val_fix['no_po'],
																'id_bahan_kemas'		=> $val_fix['id_bahan_kemas'],
																'kuantitas_po'			=> $kuantitas,
																'selisih_po_produksi'	=> 0,
																'harga_po'				=> $harga,
					...
					// Foreach
					if ($validasi > 0) {
						$msg = array(	'sukses'	=> $baris.' Data Berhasil Import PO ke Data PO!',
										'pesan_error'	=> $msg_err
			    			);
				    	echo json_encode($msg);	
					}else{
