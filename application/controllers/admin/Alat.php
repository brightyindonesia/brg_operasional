...
class Alat extends CI_Controller
{
	...
	public function proses_ocr()
	{
		...
		if (!empty($_FILES)) {
			if (count($_FILES['photo']['name']) <= 55) {
				...
				$msg  = array(
					'sukses'	=> 'Gambar berhasil di Convert ke Teks!',
					'nama'		=> $nama,
					'hp'		=> $hp,
					'resi'		=> $resi,
	...
	public function proses_ocr_by_convertpdf()
	{
		$dataOCR = array();
		$pecahanOCR = array();
		$validasiOCR = array();
		...
		if (count($ex_jpg) <= 55) {
			foreach ($ex_jpg as $val_jpg) {
				// Mendeteksi OS yang digunakan
				if (strtoupper(substr(PHP_OS, 0, 3)) === 'WIN') {
					$ocr = new TesseractOCR(base_url('uploads/hasil_convertpdf/' . $val_jpg));
					$ocr->executable('C:\Program Files\Tesseract-OCR\tesseract.exe');
					$ocr->lang('eng');
					$content = $ocr->run();
				...
				// Hapus Gambar Apabila sudah di CONVERT ke TEXT
				$dir        = "./uploads/hasil_convertpdf/" . $val_jpg;
			...
			$msg  = array(
				'validasi'	=> 'Tidak ada Gambar yang diproses OCR!'
			);
		...
		if ($filter == 'no') {
			if (count($nama) == count($hp) && count($hp) == count($resi) && count($resi) == count($nama)) {
				for ($main = 0; $main < count($nama); $main++) {
					$storeExport[$main] = array(
						'nama_penerima'	=> $nama[$main],
						'hp_penerima'	=> $hp[$main],
	...
	public function compress_convertpdf($pdf, $jpg)
	{
		...
		// Hapus .ZIP Apabila sudah di Compress
		$dir        = "./" . $zip_name;
