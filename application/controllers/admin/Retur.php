...
class Retur extends CI_Controller
{
	...
	public function __construct()
	{
		...
		$this->data['export_action'] = base_url('admin/retur/export');
	...
	public function data_retur()
	{
		...
		$this->data['get_all_followup'] = array(
			'semua'	=> '- Semua Data-',
			'0' 		=> 'Belum difollow up',
			'1' 		=> 'Sudah difollow up',
		);
	...
	public function ubah_proses()
	{
		...
		if ($this->form_validation->run() === FALSE) {
			...
			$updateRetur = array(
				'keterangan_retur' 	=> str_replace(';', ',', $this->input->post('keterangan'))
			);
	...
	public function proses_stok_aktif()
	{
		...
		$tambahStok = array(
			'qty_produk' 		=> $row_produk->qty_produk + $qty_aktif
		);
		...
		$kurangiStok = array(
			'qty_retur' 		=> $row_stok->qty_retur - $qty_aktif
		);
	...
	public function proses($id)
	{
		...
		if ($this->data['retur']) {
			...
			foreach ($this->data['produk'] as $cek) {
				...
				if (!count($cekPaket) > 0) {
					// echo $cek->id_produk;
					$this->data['array_produk'][] = array(
						'id_produk'	=> $cek->id_produk,
	...
	function hapus($id = '')
	{
		...
		if ($delete) {
			$updatePesanan = array(
				'id_status_transaksi' => $delete->status_transaksi
			);
	...
	public function export_list_retur($kurir, $toko, $status, $periodik, $followup)
	{
		$start = substr($periodik, 0, 10);
		$end = substr($periodik, 17, 24);
		$data['title']	= "Export Data Retur Per Tanggal " . $start . " - " . $end . "_" . date("H_i_s");
		$data['resi'] = $this->Retur_model->get_datatable_all($kurir, $toko, $status, $start, $end, $followup);

		// PHPOffice
		$spreadsheet = new Spreadsheet();
		$sheet = $spreadsheet->getActiveSheet();

		$sheet->setCellValue('A1', 'nomor_retur');
		$sheet->setCellValue('B1', 'tgl_retur');
		$sheet->setCellValue('C1', 'nomor_penjualan');
		$sheet->setCellValue('D1', 'nama_toko');
		$sheet->setCellValue('E1', 'nama_penerima');
		$sheet->setCellValue('F1', 'hp_penerima');
		$sheet->setCellValue('G1', 'alamat_penerima');
		$sheet->setCellValue('H1', 'kurir');
		$sheet->setCellValue('I1', 'nomor_resi');
		...
		foreach ($data['resi'] as $list) {
			// Nomor Resi
			if (is_numeric($list->nomor_retur)) {
				if (strlen($list->nomor_retur) < 15) {
					$sheet->getStyle('A' . $rowCount)->getNumberFormat()->setFormatCode(NumberFormat::FORMAT_NUMBER);
					$sheet->SetCellValue('A' . $rowCount, $list->nomor_retur);
				} else {
					...
					$sheet->setCellValueExplicit('A' . $rowCount, $list->nomor_retur, \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
				}
			} else {
				$sheet->getStyle('A' . $rowCount)->getNumberFormat()->setFormatCode(NumberFormat::FORMAT_TEXT);
				$sheet->SetCellValue('A' . $rowCount, $list->nomor_retur);
			...
			$sheet->SetCellValue('B' . $rowCount, $list->tgl_penjualan);
			...
			$sheet->SetCellValue('E' . $rowCount, $list->nama_penerima);
			...
			if (is_numeric($list->hp_penerima)) {
				if (strlen($list->hp_penerima) < 15) {
					...
					if ($firstCharacter == '0') {
						...
						$sheet->setCellValueExplicit('F' . $rowCount, $edit_no, \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
						...
						if ($ceknoldi62 == '620') {
							...
							$sheet->setCellValueExplicit('F' . $rowCount, substr_replace($list->hp_penerima, "62", 0, 3), \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
						} else {
							$sheet->getStyle('F' . $rowCount)->getNumberFormat()->setFormatCode(NumberFormat::FORMAT_TEXT);
							// The old way to force string. NumberFormat::FORMAT_TEXT is not
							// enough.
							// $formatted_value .= ' ';
							// $sheet->SetCellValue('AD' . $rowCount, "'".$formatted_value);
							$sheet->setCellValueExplicit('F' . $rowCount, $list->hp_penerima, \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
						}
					}
				} else {
					$firstCharacter = substr($list->hp_penerima, 0, 1);
				...
				if ($firstCharacter == '0') {
					$edit_no = substr_replace($list->hp_penerima, "62", 0, 1);
					$sheet->getStyle('F' . $rowCount)->getNumberFormat()->setFormatCode(NumberFormat::FORMAT_TEXT);
					$sheet->SetCellValue('F' . $rowCount, $edit_no);
				} else if ($firstCharacter == 'F') {
					$ceknoldi62 = substr($list->hp_penerima, 0, 3);
					if ($ceknoldi62 == '620') {
						$sheet->getStyle('F' . $rowCount)->getNumberFormat()->setFormatCode(NumberFormat::FORMAT_TEXT);
						$sheet->SetCellValue('F' . $rowCount, substr_replace($list->hp_penerima, "62", 0, 3));
					} else {
						$sheet->getStyle('F' . $rowCount)->getNumberFormat()->setFormatCode(NumberFormat::FORMAT_TEXT);
						$sheet->SetCellValue('F' . $rowCount, $list->hp_penerima);
					}
				}
			}

			$sheet->SetCellValue('G' . $rowCount, $list->alamat_penerima);
			$sheet->SetCellValue('H' . $rowCount, $list->nama_kurir);
			...
			if (is_numeric($list->nomor_resi)) {
				if (strlen($list->nomor_resi) < 15) {
					...
					$sheet->SetCellValue('I' . $rowCount, $list->nomor_resi);
				} else {
					...
					// $formatted_value .= ' ';
					// $sheet->SetCellValue('B' . $rowCount, "'".$formatted_value);
					$sheet->setCellValueExplicit('I' . $rowCount, $list->nomor_resi, \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
				}
			} else {
				$sheet->getStyle('I' . $rowCount)->getNumberFormat()->setFormatCode(NumberFormat::FORMAT_TEXT);
				$sheet->SetCellValue('I' . $rowCount, $list->nomor_resi);
