...
class Timeline_bahan extends CI_Controller {
    ...
    {
    	$i = 1;
        $list = $this->Timeline_bahan_model->get_datatables();
        $dataJSON = array();
        foreach ($list as $data) {
            ...
            $row = array();
            $row['no'] = $i;
            $row['nomor_timeline'] = $data->no_timeline_bahan;
            $row['nama_vendor'] = $data->nama_vendor;
            $row['jumlah'] = $data->total_bahan." / ".$data->total_bahan_jadi;
            $row['action'] = $action;
            $row['detail'] = $detail;
        ...
        $output = array(
            "recordsTotal" => $this->Timeline_bahan_model->count_all(),
            "recordsFiltered" => $this->Timeline_bahan_model->count_filtered(),
            "data" => $dataJSON,
        );
	...
	public function industry_proses()
	{
		...
		for ($n=0; $n < $len; $n++)
        {
          	$dataDetail 	= array(	'no_timeline_bahan'					=> $nomor_produksi,
          									'no_po'								=> $decode_po[$n],
											'id_jenis_timeline'					=> 2,
											'id_bahan_kemas'					=> $decode_id[$n],
											'start_date_detail_timeline_bahan'	=> $start_date,
          	...
          	$this->Timeline_bahan_model->insert_detail($dataDetail);
	...
	public function hapus_detail($id)
	{
		...
		if(isset($this->data['detail']))
		{
			if ($this->data['detail']->id_jenis_timeline == 4) {
				$no_bahan_hapus = $this->data['detail']->no_timeline_bahan;
				...
				redirect('admin/timeline_bahan/history/'.base64_encode($no_bahan_hapus));
	...
	public function hapus($id)
	{
		...
		if (isset($this->data['timeline'])) {
			if ($this->data['timeline']->status_timeline_bahan == 0) {
				...
				$this->Timeline_bahan_model->update_detailPO($no_po, $DataUpdateDetailPO);

				write_log();

				$this->Timeline_bahan_model->delete_detail_by_timeline($this->data['timeline']->no_timeline_bahan);
