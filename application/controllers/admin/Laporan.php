...
class Laporan extends CI_Controller
{
	...
	public function export_data_penjualan($users, $periodik)
	{
		...
		$users = $this->Auth_model->get_by_id($users);
		if ($users->usertype == 1 or $users->usertype == 13) {
	...
	public function export_data_bruto($users, $periodik)
	{
		date_default_timezone_set("Asia/Jakarta");
		$now = date('Y-m-d H:i:s');
		$start = substr($periodik, 0, 10);
		$end = substr($periodik, 17, 27);
		$isi = "Export Data Bruto Penjualan";
		$users = $this->Auth_model->get_by_id($users);
		if ($users->usertype == 1 or $users->usertype == 13) {
			$data['title']	= "Export Data Bruto Penjualan Per Tanggal " . $start . " - " . $end . "_" . date("H_i_s");
			$data['pendapat'] = $this->Laporan_model->get_pendapat_periodik_penjualan($start, $end);

			// PHPOffice
			$spreadsheet = new Spreadsheet();
			$sheet = $spreadsheet->getActiveSheet();

			$sheet->setCellValue('A1', 'nama_toko');
			$sheet->setCellValue('B1', 'bruto');
			// set Row
			$rowCount = 2;

			foreach ($data['pendapat'] as $list) {
				$sheet->SetCellValue('A' . $rowCount, $list->nama_toko);
				$sheet->SetCellValue('B' . $rowCount, $list->bruto);
				$rowCount++;
			}
	...
	public function export_data_margin($users, $periodik)
	{
		date_default_timezone_set("Asia/Jakarta");
		$now = date('Y-m-d H:i:s');
		$start = substr($periodik, 0, 10);
		$end = substr($periodik, 17, 27);
		$isi = "Export Data Margin Penjualan";
		$users = $this->Auth_model->get_by_id($users);
		if ($users->usertype == 1 or $users->usertype == 13) {
			$data['title']	= "Export Data Margin Penjualan Per Tanggal " . $start . " - " . $end . "_" . date("H_i_s");
			$data['pendapat'] = $this->Laporan_model->get_pendapat_periodik_penjualan($start, $end);
			...
			$sheet->setCellValue('A1', 'nama_toko');
			$sheet->setCellValue('B1', 'total_jual');
			$sheet->setCellValue('C1', 'total_ongkir');
			$sheet->setCellValue('D1', 'total_hpp');
			$sheet->setCellValue('E1', 'margin');
			// set Row
			$rowCount = 2;
			...
			foreach ($data['pendapat'] as $list) {
				$sheet->SetCellValue('A' . $rowCount, $list->nama_toko);
				$sheet->SetCellValue('B' . $rowCount, $list->bruto);
				$sheet->SetCellValue('C' . $rowCount, $list->tot_ongkir);
				$sheet->SetCellValue('D' . $rowCount, $list->tot_hpp);
				$sheet->SetCellValue('E' . $rowCount, $list->fix);
				$rowCount++;
			}
	...
	public function export_data_pending($users, $periodik)
	{
		date_default_timezone_set("Asia/Jakarta");
		$now = date('Y-m-d H:i:s');
		$start = substr($periodik, 0, 10);
		$end = substr($periodik, 17, 27);
		$isi = "Export Data Pending Payment Penjualan";
		$users = $this->Auth_model->get_by_id($users);
		if ($users->usertype == 1 or $users->usertype == 13) {
			$data['title']	= "Export Data Pending Payment Penjualan Per Tanggal " . $start . " - " . $end . "_" . date("H_i_s");
			$data['pendapat'] = $this->Laporan_model->get_pendapat_periodik_penjualan($start, $end);
			...
			$sheet->setCellValue('A1', 'nama_toko');
			$sheet->setCellValue('B1', 'total_pending_payment');
			$sheet->setCellValue('C1', 'total_diterima');
			$sheet->setCellValue('D1', 'biaya_lainnya');
			// set Row
			$rowCount = 2;

			foreach ($data['pendapat'] as $list) {
				$sheet->SetCellValue('A' . $rowCount, $list->nama_toko);
				$sheet->SetCellValue('B' . $rowCount, $list->fix);
				$sheet->SetCellValue('C' . $rowCount, $list->diterima);
				if ($list->diterima == 0) {
					$sheet->SetCellValue('D' . $rowCount, '-');
				} else {
					$sheet->SetCellValue('D' . $rowCount, $list->biaya_lainnya);
				}
				$rowCount++;
			}
