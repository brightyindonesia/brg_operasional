...
class Kategori_kasus extends CI_Controller {
	...
	function export_kategori_kasus()
	{
		$data['title']	= "Export Data Kategori Kasus_".date("Y_m_d");
		$data['kategori_kasus']	= $this->Kategori_kasus_model->get_all();
		...
		$sheet->setCellValue('A1', 'id_kategori_kasus');
		$sheet->setCellValue('B1', 'nama_kategori_kasus');

        // set Row
        $rowCount = 2;
        foreach ($data['kategori_kasus'] as $list) {
        	// Nomor Pesanan
            ...
            $sheet->SetCellValue('A' . $rowCount, $list->id_kategori_kasus);
            $sheet->SetCellValue('B' . $rowCount, $list->nama_kategori_kasus);
			...
			foreach ($reader->getSheetIterator() as $sheet) {
				...
				if ($numSheet == 0) {
					foreach ($sheet->getRowIterator() as $row) {
						if ($numRow == 1) {
							if ($row->getCellAtIndex(0) != 'ID Kategori Kasus' || $row->getCellAtIndex(1) != 'Nama Kategori Kasus') {
								$reader->close();
