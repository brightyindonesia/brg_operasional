...
class Jenis_timeline extends CI_Controller {
	...
	function hapus_dipilih()
	{
		...
		$jenis = $this->input->post('ids');
		// echo $produk;

		$this->Jenis_timeline_model->delete_in($jenis);
	...
	function export_jenis_timeline()
	{
		$data['title']	= "Export Data Jenis Timeline_".date("Y_m_d");
		$data['jenis_timeline']	= $this->Jenis_timeline_model->get_all();
		...
		$sheet->setCellValue('A1', 'id_jenis_timeline');
		$sheet->setCellValue('B1', 'kode_jenis_timeline');
		$sheet->setCellValue('C1', 'nama_jenis_timeline');

        // set Row
        $rowCount = 2;
        foreach ($data['jenis_timeline'] as $list) {
        	// Nomor Pesanan
            ...
            $sheet->SetCellValue('A' . $rowCount, $list->id_jenis_timeline);
            $sheet->SetCellValue('B' . $rowCount, $list->kode_jenis_timeline);
            $sheet->SetCellValue('C' . $rowCount, $list->nama_jenis_timeline);
