...
class Toko extends CI_Controller {
	...
	function export_toko()
	{
		...
		$sheet->setCellValue('A1', 'id_toko');
		$sheet->setCellValue('B1', 'id_jenis_toko');
		$sheet->setCellValue('C1', 'nama_toko');
        ...
        foreach ($data['toko'] as $list) {
            ...
            $sheet->SetCellValue('A' . $rowCount, $list->id_toko);
            $sheet->SetCellValue('B' . $rowCount, $list->id_jenis_toko);
            $sheet->SetCellValue('C' . $rowCount, $list->nama_toko);
			...
			foreach ($reader->getSheetIterator() as $sheet) {
				...
				if ($numSheet == 0) {
					foreach ($sheet->getRowIterator() as $row) {
						if ($numRow == 1) {
							if ($row->getCellAtIndex(0) != 'ID Toko' || $row->getCellAtIndex(1) != 'ID Jenis Toko' || $row->getCellAtIndex(2) != 'Nama Toko') {
								$reader->close();
