...
class Produk extends CI_Controller {
	...
	function ubah_proses()
	{
		...
		// if ($this->input->post('paket') != '') {
		// 	if (isset($cek_paket)) {
				if ($cek_paket->paket_utama != $this->input->post('paket')) {
					if ($this->input->post('paket') == '') {
	...
	function generatehpp($id = '')
	{
    	...
    	if (count($cek_propak) > 0) {
			...
			foreach ($cek_propak as $val_propak) {
				...
				$harga =+ $harga + ($val_propak->qty_pakduk * $val_propak->harga_produk);
			}
			...
			$this->session->set_flashdata('message', '<div class="alert alert-success">Data saved succesfully. Generate HPP and Product Price '.$detail_produk->nama_produk.': <b>'.$total.'</b> and <b>'.$harga.'</b></div>');
			redirect('admin/produk');
		}		
	}
	...
	function generatehpp_dipilih()
	{
    	...
    	if (count($cek_propak) > 0) {
			...
			foreach ($cek_propak as $val_propak) {
				if ($konfirm_utama == $val_propak->produk_utama) {
					$total = $total + ((int)$val_propak->qty_pakduk * (int)$val_propak->hpp_produk);
					$harga = $harga + ((int)$val_propak->qty_pakduk * (int)$val_propak->harga_produk);
					
					$arr_data[$val_propak->produk_utama] = array(	'hpp_produk'   => $total,
				        											'harga_produk' => $harga
					);

					$konfirm_utama = $val_propak->produk_utama;	
				}else{
					$total = 0;
					$harga = 0;
			...
			foreach ($arr_data as $key_data => $field) {
				foreach ($field as $val_field => $result) {
					$data = array(
				        $val_field   => $result
					);

					$this->Produk_model->update($key_data,$data);
		...
		$pesan = "Berhasil digenerate hpp dan harga produk!";	
			...
			foreach ($reader->getSheetIterator() as $sheet) {
				...
				if ($numSheet == 0) {
					foreach ($sheet->getRowIterator() as $row) {
						if ($numRow == 1) {
							if ($row->getCellAtIndex(0) != 'ID Produk' || $row->getCellAtIndex(1) != 'ID Satuan' || $row->getCellAtIndex(2) != 'ID SKU' || $row->getCellAtIndex(3) != 'Sub SKU' || $row->getCellAtIndex(4) != 'Nama Produk' || $row->getCellAtIndex(5) != 'Qty Produk' || $row->getCellAtIndex(6) != 'Hpp Produk' || $row->getCellAtIndex(7) != 'Harga Produk') {
								$reader->close();
						...
						if ($numRow > 1) {
							$data 	= array(	'id_produk'			=> $row->getCellAtIndex(0),
												...
												'harga_produk'		=> $row->getCellAtIndex(7),
							);
	...
	function get_hpp_harga_by_paket()
	{
		$cek_propak = $this->Paket_model->get_pakduk_produk_by_produk_by_paket($this->input->post('paket'));
    	if (count($cek_propak) > 0) {
    		$total = 0;
    		$harga = 0;
			// echo print_r($cek_propak)."<br>";
