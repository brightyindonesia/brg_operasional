			...
			foreach ($reader->getSheetIterator() as $sheet) {
				...
				if ($numSheet == 0) {
					foreach ($sheet->getRowIterator() as $row) {
						if ($numRow == 1) {
							if ($row->getCellAtIndex(0) != 'ID Vendor' || $row->getCellAtIndex(1) != 'Nama Vendor' || $row->getCellAtIndex(2) != 'Alamat Vendor' || $row->getCellAtIndex(3) != 'No. HP Vendor' || $row->getCellAtIndex(4) != 'No. Telepon Vendor') {
								$reader->close();
