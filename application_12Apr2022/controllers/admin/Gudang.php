...
class Gudang extends CI_Controller {
	...
	function export_gudang()
	{
		...
		$sheet->setCellValue('A1', 'id_gudang');
		$sheet->setCellValue('B1', 'nama_gudang');
		$sheet->setCellValue('C1', 'alamat_gudang');
        ...
        foreach ($data['gudang'] as $list) {
            ...
            $sheet->SetCellValue('A' . $rowCount, $list->id_gudang);
            $sheet->SetCellValue('B' . $rowCount, $list->nama_gudang);
            $sheet->SetCellValue('C' . $rowCount, $list->alamat_gudang);
			...
			foreach ($reader->getSheetIterator() as $sheet) {
				...
				if ($numSheet == 0) {
					foreach ($sheet->getRowIterator() as $row) {
						if ($numRow == 1) {
							if ($row->getCellAtIndex(0) != 'ID Gudang' || $row->getCellAtIndex(1) != 'Nama Gudang' || $row->getCellAtIndex(2) != 'Alamat Gudang') {
								$reader->close();
