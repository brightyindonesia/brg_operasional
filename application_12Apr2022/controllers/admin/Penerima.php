...
class Penerima extends CI_Controller {
	...
	function export_penerima()
	{
		...
		$sheet->setCellValue('A1', 'id_penerima');
		$sheet->setCellValue('B1', 'nama_penerima');
		$sheet->setCellValue('C1', 'alamat_penerima');
		$sheet->setCellValue('D1', 'no_hp_penerima');
		$sheet->setCellValue('E1', 'no_telpon_penerima');

        // set Row
        $rowCount = 2;
        foreach ($data['penerima'] as $list) {
        	// Nomor Pesanan
            ...
            $sheet->SetCellValue('A' . $rowCount, $list->id_penerima);
            $sheet->SetCellValue('B' . $rowCount, $list->nama_penerima);
            $sheet->SetCellValue('C' . $rowCount, $list->alamat_penerima);
            $sheet->SetCellValue('D' . $rowCount, $list->no_hp_penerima);
            $sheet->SetCellValue('E' . $rowCount, $list->no_telpon_penerima);
			...
			foreach ($reader->getSheetIterator() as $sheet) {
				...
				if ($numSheet == 0) {
					foreach ($sheet->getRowIterator() as $row) {
						if ($numRow == 1) {
							if ($row->getCellAtIndex(0) != 'ID Penerima' || $row->getCellAtIndex(1) != 'Nama Penerima' || $row->getCellAtIndex(2) != 'Alamat Penerima' || $row->getCellAtIndex(3) != 'No. HP Penerima' || $row->getCellAtIndex(4) != 'No. Telepon Penerima') {
								$reader->close();
