...
class Bahan_kemas extends CI_Controller {
	...
	public function tambah_proses()
	{
		...
		$this->form_validation->set_rules('venmas_access_id[]', 'Daftar Vendor', 'required',
			array(	'required' 		=> '%s harus diisi!')
		);
	...
	function export_bahan_kemas()
	{
		...
		$sheet->setCellValue('A1', 'id_bahan_kemas');
		$sheet->setCellValue('B1', 'id_satuan');
		$sheet->setCellValue('C1', 'kode_sku_bahan_kemas');
		$sheet->setCellValue('D1', 'nama_bahan_kemas');
		$sheet->setCellValue('E1', 'qty_bahan_kemas');
		$sheet->setCellValue('F1', 'keterangan');

        // set Row
        $rowCount = 2;
        foreach ($data['bahan_kemas'] as $list) {
        	// Nomor Pesanan
            ...
            $sheet->SetCellValue('A' . $rowCount, $list->id_bahan_kemas);
            $sheet->SetCellValue('B' . $rowCount, $list->id_satuan);
            $sheet->SetCellValue('C' . $rowCount, $list->kode_sku_bahan_kemas);
            $sheet->SetCellValue('D' . $rowCount, $list->nama_bahan_kemas);
            $sheet->SetCellValue('E' . $rowCount, $list->qty_bahan_kemas);
            $sheet->SetCellValue('F' . $rowCount, $list->keterangan);
			...
			foreach ($reader->getSheetIterator() as $sheet) {
				...
				if ($numSheet == 0) {
					foreach ($sheet->getRowIterator() as $row) {
						if ($numRow == 1) {
							if ($row->getCellAtIndex(0) != 'ID Bahan Kemas' || $row->getCellAtIndex(1) != 'ID Satuan' || $row->getCellAtIndex(2) != 'Kode SKU' || $row->getCellAtIndex(3) != 'Nama Bahan Kemas' || $row->getCellAtIndex(4) != 'Qty' || $row->getCellAtIndex(5) != 'Keterangan') {
								$reader->close();
