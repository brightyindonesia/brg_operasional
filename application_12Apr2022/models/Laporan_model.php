...
class Laporan_model extends CI_Model {
  ...
  function get_all_toko_only()
  {
    ...
    $this->db->where_not_in($this->id_toko, array(20, 25, 26, 30, 31));
  ...
  public function get_pendapat_periodik_penjualan($first, $last)
  {
    ...
    $this->db->select('((sum(total_harga)) - (sum(total_hpp))) - sum(jumlah_diterima) as biaya_lainnya');
    $this->db->join('toko', 'toko.id_toko = penjualan.id_toko');
  ...
  //                       ));
    ...
    $this->db->group_by('penjualan.id_toko');
    return $this->db->get('penjualan')->result();
  }
