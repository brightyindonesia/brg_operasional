...
<div class="wrapper">
          ...
          <div class="box box-primary no-border">
            <div class="box-header">
              <div class="row">
                ...
                <div class="col-sm-4">
                  <div class="form-group">
                    <label>Sub SKU Per Produk</label>
                    <button id="btn-sub-sku-produk" class="btn btn-sm btn-success form-control" style="float: right;"><i class="fa fa-file-excel-o" style="margin-right: 5px"></i> Cetak</button>
                  </div>
                </div>

                <div class="col-sm-4">
                  <div class="form-group">
                    <label>HPP Produk Per Produk</label>
                    <button id="btn-hpp-produk" class="btn btn-sm btn-success form-control" style="float: right;"><i class="fa fa-file-excel-o" style="margin-right: 5px"></i> Cetak</button>
                  </div>
                </div>
              </div>
            </div>
  ...
  $(document).ready( function () {
    ...
    $('#btn-sub-sku-produk').click(function(){
      var check = confirm("Are you sure you ?");  
      if(check == true){
        var sku = $('#sku').val();
        var users = <?php echo $this->session->userdata('id_users'); ?>;

        window.open("<?php echo base_url() ?>admin/laporan/export_sub_sku_produk/"+users+"/"+sku,+"_self");
      }
    });

    $('#btn-hpp-produk').click(function(){
      var check = confirm("Are you sure you ?");  
      if(check == true){
        var sku = $('#sku').val();
        var users = <?php echo $this->session->userdata('id_users'); ?>;

        window.open("<?php echo base_url() ?>admin/laporan/export_hpp_produk/"+users+"/"+sku,+"_self");
      }
    });    
  });
  </script>
