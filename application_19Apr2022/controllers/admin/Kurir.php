...
class Kurir extends CI_Controller {
	...
	function hapus_dipilih()
	{
		...
		$this->Kurir_model->delete_in($produk);
	...
	function export_kurir()
	{
		...
		$sheet->setCellValue('A1', 'id_kurir');
		$sheet->setCellValue('B1', 'nama_kurir');
        ...
        foreach ($data['kurir'] as $list) {
            ...
            $sheet->SetCellValue('A' . $rowCount, $list->id_kurir);
            $sheet->SetCellValue('B' . $rowCount, $list->nama_kurir);
			...
			foreach ($reader->getSheetIterator() as $sheet) {
				...
				if ($numSheet == 0) {
					foreach ($sheet->getRowIterator() as $row) {
						if ($numRow == 1) {
							if ($row->getCellAtIndex(0) != 'ID Kurir' || $row->getCellAtIndex(1) != 'Nama Kurir') {
								$reader->close();
