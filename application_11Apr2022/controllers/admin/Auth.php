...
class Auth extends CI_Controller{
  ...
  function update($id)
  {
    ...
    if($this->data['user'])
    {
      ...
      $this->data['password'] = [
        'name'          => 'password',
        'id'            => 'password',
        'placeholder'   => 'Kosongkan apabila tidak ingin merubah password!',
        'class'         => 'form-control',
        'autocomplete'  => 'off',
      ];
      $this->data['usertype'] = [
  ...
  function login()
  {
    ...
    if($this->form_validation->run() === TRUE)
    {
      ...
      else
      {
        ...
        // Memisahkan apabila ini untuk Admin CS (hanya melihat Data Resi)
        if ($row->usertype == 12) {
          redirect('admin/resi');
        }else{
          redirect('admin/dashboard');
      ...
      else
      {
        ...
        else
        {
          ...
          $mailContent = '
            ...
            In a few moment ago, we received your request to reset your account password <br>
            To reset your password, please click the following link below:
            <p><a href="'.base_url('admin/auth/reset_password/'.$hash).'">'.$hash.'</a></p>
            <p>If you did not initiate this request, you may safely ignore this message. The request will automatically expired.</p>
            <hr>
            <b>THIS EMAIL IS CREATED AUTOMATICALLY. PLEASE DO NOT SEND A REPLY TO THIS EMAIL.</b>
            <hr>
  ...
  function reset_password($code = NULL)
  {
    ...
    else
    {
      ...
      $this->data['action'] = 'admin/auth/reset_password_process/'.$code;
  ...
  function reset_password_process()
  {
    ...
    {
      $get_user = $this->Auth_model->get_by_code_forgotten_row($this->input->post('code_forgotten'));
      if (isset($get_user)) {
        $password = password_hash($this->input->post('new_password'), PASSWORD_BCRYPT);
        ...
        $this->Auth_model->update($get_user->id_users, $data);
