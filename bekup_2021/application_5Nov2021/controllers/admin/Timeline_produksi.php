...
class Timeline_produksi extends CI_Controller {
	...
	public function history($id)
	{
		...
		$this->data['bahan_kemas']			= $this->Timeline_produksi_model->get_posi_bahan_jenis_timeline_by_id(2, base64_decode($id));
		$this->data['get_all_sku'] 			= $this->Timeline_produksi_model->get_all_sku();
	    $this->data['get_all_kategori']		= $this->Timeline_produksi_model->get_all_kategori(); 
	...
	public function industry_proses()
	{
        ...
        for ($n2 = 0; $n2 < $len; $n2++) {
        	$row_posi_po = $this->Timeline_produksi_model->get_posi_by_po_bahan_produksi($decode_po[$n2], $decode_id[$n2], $nomor_produksi);
        	if ($row_posi_po) {
        		$tambah_qty = $row_posi_po->qty_detail_timeline_produksi + $decode_qty[$n2];
        		$tambah_qty_selisih = $row_posi_po->qty_selisih_detail_timeline_produksi + $decode_qty[$n2];

				$updateQtyPosi = array( 'qty_selisih_detail_timeline_produksi' 	=> $tambah_qty_selisih,
										'qty_detail_timeline_produksi'	 		=> $tambah_qty
				);
				...
				if ($get_detail) {
					$dataPosi	= array(	'id_detail_timeline_produksi'			=> $last_row->id_detail_timeline_produksi,
											...
											'qty_detail_timeline_produksi'			=> $decode_qty[$n2],
	...
	public function delivery_proses()
	{
        ...
        // Insert Propo
      //   $last_row = $this->Timeline_produksi_model->get_last_detail_timeline(3, $nomor_produksi);
      ...
      //   	$get_detail_bahan = $this->Timeline_produksi_model->get_bahan_posi_by_bahan_id_detail($nomor_produksi, $decode_dr_id_detail[$r], $decode_dr_id[$r]);
      ...
      //   		$sisa_nambah_qty = $get_detail_bahan->sisa_detail_timeline_produksi + $decode_dr_sisa[$r];
      //   	}
	...
	public function add_posi_delivery($id)
	{
		...
		if ($this->data['detail_timeline']) {
			...
			$this->data['reject']				= $this->Timeline_produksi_model->get_bahan_posi_by_bahan($this->data['detail_timeline']->no_timeline_produksi);
			$this->data['get_all_sku'] 			= $this->Timeline_produksi_model->get_all_sku();
		    $this->data['get_all_kategori']		= $this->Timeline_produksi_model->get_all_kategori(); 

	    	// if ($this->data['timeline']->total_bahan_jadi <= $this->data['timeline']->total_bahan) {
		    	$this->data['page_title'] = 'Add Production Material to Detail Delivery: '.$this->data['detail_timeline']->no_timeline_produksi;
	...
	public function hapus_detail($id)
	{
		...
		if(isset($this->data['detail']))
		{
			...
			if ($this->data['detail']->id_jenis_timeline == 4) {
				...
				$this->session->set_flashdata('message', '<div class="alert alert-success">Data deleted successfully</div>');
				$this->history(base64_encode($no_produksi_hapus));
				...
				if ($get_propo) {
					foreach ($get_propo as $val_propo) {
						...
						$kurang_sisa = $row_posi_po->sisa_detail_timeline_produksi - $val_propo->sisa_detail_timeline_produksi;
						$kurang_terpakai = $row_posi_po->selisih_detail_timeline_produksi - $val_propo->selisih_detail_timeline_produksi;

						$updateQtyPosi = array( 'qty_detail_timeline_produksi' 		=> $tambah_qty,
												'terpakai_detail_timeline_produksi' => $kurang_terpakai,
												'sisa_detail_timeline_produksi' 	=> $kurang_sisa,
												'selisih_detail_timeline_produksi' 	=> $kurang_terpakai,
						);
				...
				$cek_jenis_timeline = $this->Timeline_produksi_model->check_posi_jenis_timeline(3);
				if ($cek_jenis_timeline) {
					$this->session->set_flashdata('message', '<div class="alert alert-danger">Production Timeline submissions must be cleared first</div>');
					$this->history(base64_encode($no_produksi_hapus));
				}else{
					$get_propo = $this->Timeline_produksi_model->get_popro_posi(base64_decode($id));
					foreach ($get_propo as $val_propo) {
						$get_po = $this->Timeline_produksi_model->get_all_po_by_po_bahan_row($val_propo->no_po, $val_propo->id_bahan_kemas);

						if ($get_po) {
							$TambahJumlahPO = array( 'total_selisih_po_produksi' 	=> $get_po->total_selisih_po_produksi + $val_propo->qty_detail_timeline_produksi,
				          	);
				          	...
				          	$TambahJumlahdetailPO = array( 'selisih_po_produksi' 		=> $get_po->selisih_po_produksi + $val_propo->qty_detail_timeline_produksi,
				          	);
					...
					$this->history(base64_encode($this->data['detail']->no_timeline_produksi));
				}
			}else{
				$this->Timeline_produksi_model->delete_detail(base64_decode($id));
				write_log();
