...
class Timeline_produksi extends CI_Controller {
	...
	public function edit_info($id)
	{
		...
		if ($this->data['detail_timeline']) {
		    ...
		    {
		    	// echo print_r($this->data['daftar_bahan_kemas']);
		    	$this->data['page_title'] = 'Edit Detail Info: '.$this->data['detail_timeline']->no_timeline_produksi;
		    	$this->data['action']     = 'admin/timeline_produksi/proses_edit_info';
		    	$this->data['nomor_produksi'] = [
			        'class'         => 'form-control',
					'readonly' 		=> '' 
	      		];
			    ...
			    $this->load->view('back/timeline_produksi/timeline_produksi_info_edit', $this->data);
		    }else{
		    	$this->session->set_flashdata('message', '<div class="alert alert-danger">Data not found</div>');
		    	redirect('admin/timeline_produksi/timeline');
		    }
	...
	public function info_proses_edit()
	{
		// Ambil Data
		$i = $this->input;
		...
		$keterangan	 	= $i->post('keterangan');
      	...
      	$updateDetail 		= array(	'start_date_detail_timeline_produksi'	=> $start_date,
										'end_date_detail_timeline_produksi'		=> $end_date,
										'ket_detail_timeline_produksi'			=> $keterangan,
								);

      	$this->Timeline_produksi_model->updateDetailTimeline($id_detail,$updateDetail);
        ...
        $pesan = "Informasi Timeline berhasil diubah!";	
    	$msg = array(	'sukses'	=> $pesan,
    					'produksi'  => base64_encode($nomor_produksi) 
    			);
    	echo json_encode($msg);
		...
		for ($n=0; $n < $len_bahan; $n++)
        {
			...
			// if (!$get_detail) {
				$dataDetail	= array(	'no_timeline_produksi'					=> $nomor_produksi,
										...
										'start_date_detail_timeline_produksi'	=> $start_date,
										'end_date_detail_timeline_produksi'		=> $end_date,
										'qty_detail_timeline_produksi'			=> $decode_qty_bahan[$n],
										'harga_detail_timeline_produksi'		=> $decode_harga_bahan[$n],
										'ket_detail_timeline_produksi'			=> $keterangan,
								);
	...
	function proses_edit_industry()
	{
		...
		$dataDetail	= array(	'start_date_detail_timeline_produksi'	=> $start_date,
      	...
      	foreach ($cek_posi as $val_posi) {
      		$data_posi = $this->Timeline_produksi_model->get_posi_by_po_bahan_produksi_detail($val_posi->no_po, $val_posi->id_bahan_kemas, $val_posi->no_timeline_produksi, $val_posi->id_posi_data_access);
	...
	function proses_edit_posi_delivery()
	{
        ...
        $cek_posi = $this->Timeline_produksi_model->get_bahan_posi_by_detail($id_detail);
      	...
      	foreach ($cek_posi as $val_posi) {
      		...
      		$UpdateStok = array( 'fix_sisa_stok_pabrik'	=> $data_stok->fix_sisa_stok_pabrik + $data_posi->selisih_detail_timeline_produksi,
      							 'reject_stok_pabrik'	=> $data_stok->reject_stok_pabrik - $data_posi->selisih_detail_timeline_produksi,
      		);
      	...
      	for ($r = 0; $r < $len_dr; $r++) {
      		...
      		$UpdateStok = array( 'fix_sisa_stok_pabrik'	=> $data_stok->fix_sisa_stok_pabrik - $decode_dr_selisih[$r],
      							 'reject_stok_pabrik'	=> $data_stok->reject_stok_pabrik + $decode_dr_selisih[$r],
      		);
      		...
      		$UpdatePosi = array( 'sisa_detail_timeline_produksi'	=> $decode_dr_sisa_fix[$r],
					      		 'selisih_detail_timeline_produksi'	=> $decode_dr_selisih[$r],	      		
	      	);
	...
	public function edit_retur($id)
	{
		...
		if ($this->data['detail_timeline']) {
		    ...
		    {
		    	if (!$this->data['timeline']->total_produksi_jadi == 0) {
			    	$this->data['page_title'] = 'Edit Detail Retur: '.$this->data['detail_timeline']->no_timeline_produksi;
			    	$this->data['action']     = 'admin/timeline_bahan/proses_retur_edit';
			    	$this->data['nomor_produksi'] = [
				        'class'         => 'form-control',
						'readonly' 		=> '' 
		      		];
				    ...
				    $this->load->view('back/timeline_produksi/timeline_produksi_retur_edit', $this->data);	
		    	}else{
		    		$this->session->set_flashdata('message', '<div class="alert alert-danger">Timeline: '.$this->data['timeline']->no_timeline_produksi.'. Order not sent</div>');
			    	redirect('admin/timeline_produksi/timeline');
		    	}
	...
	public function retur_proses_edit()
	{
		$total_pesan = 0;
		...
		$detail_timeline= $this->Timeline_produksi_model->get_detail_all_by_timeline_row($id_detail);
		
		$updatePO = array(	'total_selisih_po_produksi'	=> $detail->total_selisih_po_produksi + $detail_timeline->qty_detail_timeline_produksi
    	);

    	$this->Timeline_produksi_model->updatePO($detail_timeline->no_po, $updatePO);

    	write_log();

    	$updateTimeline = array(	'total_produksi_jadi'	=> $detail->total_produksi_jadi + $detail_timeline->qty_detail_timeline_produksi
    	);
    	
    	$this->Timeline_produksi_model->updateTimeline($detail_timeline->no_timeline_produksi, $updateTimeline);

    	write_log();

    	$updateDetailPO= array( 'selisih_po_produksi'	=> $detail->selisih_po_produksi + $detail_timeline->qty_detail_timeline_produksi
    	);

    	$this->Timeline_produksi_model->update_detailPO_by_bahan($detail_timeline->no_po, $detail_timeline->bahan_id, $updateDetailPO);
		...
		for ($n=0; $n < $len; $n++)
        {
        	$detail_len 		= $this->Timeline_produksi_model->get_detail_all_by_timeline_produksi_row($nomor_produksi);
        	$updatePO = array(	'total_selisih_po_produksi'	=> $detail_len->total_selisih_po_produksi - $total_pesan
        	);
        	...
        	$updateTimeline = array(	'total_produksi_jadi'	=> $detail_len->total_produksi_jadi - $total_pesan
        	);
        	$this->Timeline_produksi_model->updateTimeline($nomor_produksi, $updateTimeline);
        	...
        	write_log();
          	...
          	$dataDetail[$n] 	= array(	'start_date_detail_timeline_produksi'	=> $start_date,
											'end_date_detail_timeline_produksi'		=> $end_date,
											'qty_detail_timeline_produksi'			=> $decode_qty[$n],
											'harga_detail_timeline_produksi'		=> $harga_detail,
											'ket_detail_timeline_produksi'			=> $fix_ket,
          	...
          	$this->Timeline_produksi_model->updateDetailTimeline($id_detail, $dataDetail[$n]);
        ...
        $pesan = "Retur Timeline berhasil diubah!";	
    	$msg = array(	'sukses'	=> $pesan,
    					'produksi'  => base64_encode($nomor_produksi)     					
    			);
    	echo json_encode($msg);
			...
			if ($this->data['detail']->id_jenis_timeline == 4) {
				...
				// }else{
					...
					foreach ($get_propo as $val_propo) {
						...
						$get_posi = $this->Timeline_produksi_model->get_posi_by_po_bahan_produksi_detail($val_propo->no_po, $val_propo->id_bahan_kemas, $val_propo->no_timeline_produksi, $val_propo->id_posi_data_access);
			          	...
			          	if ($get_stok || $get_posi) {
				          	$kurangiFixSisa = array( 'fix_sisa_stok_pabrik' 	=> $get_stok->fix_sisa_stok_pabrik + $get_posi->qty_selisih_detail_timeline_produksi,
				          							 'terpakai_stok_pabrik' 	=> $get_stok->terpakai_stok_pabrik - $get_posi->qty_selisih_detail_timeline_produksi,	
				          	);
