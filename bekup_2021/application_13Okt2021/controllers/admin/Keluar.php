...
class Keluar extends CI_Controller {
	...
	// 						try {
	// 							$cells 	   = $row->getCells();
	// 							$produk    = $this->Produk_model->get_by_id($row->getCellAtIndex(12));
	...
	// 																'alamat_penerima' 		=> $row->getCellAtIndex(7),
	...
	// 							$this->session->set_flashdata('message', '<div class="alert alert-success">'.$e->getMessage().'</div>');
	// 							redirect('admin/keluar/data_penjualan');
	// 						}
	// 					}
	// 					$numRow++;
	...
	// 				    $this->session->set_flashdata('message', '<div class="alert alert-danger">'.log_message().'</div>');
	// 				} else {
						...
						if ($numRow > 1) {
							try {
								...
								if (isset($row_detail)) {
									...
									$error = array('error' => $this->upload->display_errors());
									// return $error;
									// $this->session->set_flashdata('message', '<div class="alert alert-danger">'.print_r($error).'</div>');
									redirect('admin/keluar/data_penjualan', $this->session->set_flashdata('message', '<div class="alert alert-danger">'.$error.'</div>'));
								}
							} catch (Exception $e) {
								$reader->close();
								unlink('uploads/'.$file['file_name']);
