...
class Request_model extends CI_Model {
  ...
  public $order_data = array('tgl_request' => 'DESC'); // default order 
  ...
  {
    $this->db->join($this->table_detail, 'request.no_request = detail_request.no_request');
    $this->db->join($this->table_bahan_kemas, 'bahan_kemas.id_bahan_kemas = detail_request.id_bahan_kemas');
    $this->db->where('request.no_request', $id);
    return $this->db->get($this->table)->result();
  }
  ...
  function get_all_detail_by_id_bahan_row($id, $bahan_kemas)
  {
    $this->db->join($this->table_detail, 'request.no_request = detail_request.no_request');
    $this->db->join($this->table_bahan_kemas, 'bahan_kemas.id_bahan_kemas = detail_request.id_bahan_kemas');
    $this->db->where(array( 'request.no_request'            => $id,
                            'detail_request.id_bahan_kemas' => $bahan_kemas
                  ));
    return $this->db->get($this->table)->row();
  }
  ...
  {
    $this->db->where($this->id, $id);
    $this->db->where($this->id_bahan_kemas, $bahan_kemas);
